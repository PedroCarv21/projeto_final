...
export function PaymentPage({ onNavigate, bookingData }: PaymentPageProps) {
  ...
  const getBookingData = () => {
    ...
    // prioridade sempre ao sessionStorage
    if (sessionData) return JSON.parse(sessionData);

    // se não houver sessionStorage, use bookingData
  ...
  console.log("RoomId carregado:", booking.roomId);
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="mb-8">
          <Button
            variant="ghost"
            onClick={() => {
              if (booking.type === "room" && booking.roomId) {
                onNavigate("room-details", booking.roomId);
              } else {
                onNavigate("home");
              }
            }}
